	...
	<div>
		<div class="container">
			<div class="handle-box">
				<el-input v-model="query.keyword" placeholder="身体部位" class="handle-input mr10"></el-input>
				<el-button type="primary" :icon="Search" @click="handleSearch">搜索</el-button>
				<el-button type="primary" :icon="Plus" @click="handleAdd()">新增</el-button>
			</div>
			<el-table :data="tableData" border class="table" ref="multipleTable" header-cell-class-name="table-header">
				<el-table-column prop="id" label="ID" width="55" align="center"></el-table-column>
				<el-table-column prop="bodyName" label="身体部位"></el-table-column>
		...
		<el-dialog :title='isAdd ? "新增" : "编辑"' v-model="editVisible" width="30%">
			<el-form label-width="70px">
				...
				<el-form-item label="身体部位">
					<el-input v-model="form.bodyName"></el-input>
				</el-form-item>
			</el-form>
			<template #footer>
...
// 获取表格数据
const getData = () => {
	fetchBodyPart(query.keyword, query.pageIndex, query.pageSize).then(res => {
...
const handleDelete = (index: number, id: number) => {
	...
	ElMessageBox.confirm('确定要删除吗？', '提示', {
		...
		const { code, data, msg } = await delBodyPart(id);
		if (data == 1) {
...
const save = async () => {
	...
	if (isAdd.value) {
		const { code, data, msg } = await addBodyPart(form.bodyName);
		if (data == 1) {
		...
		const { code, data, msg } = await editBodyPart(form.id, form.bodyName);
		if (data == 1) {
